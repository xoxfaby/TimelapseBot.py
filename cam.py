...
Parameters: effectname, eff=effectname shutter=seconds night'''
  ...
  if not lowpower:
    ...
    async with message.channel.typing():
      ...
      filepath = path.join(client.dirs['live'],f'{timestampshort}_{shortname}_{message.author.id}.jpg')
...
async def cWeather(client,message,params={}):
  ...
  lat = 48.1798946
  long = 16.2698437
  blocks = ['minutely', 'hourly', 'daily', 'alerts', 'flags']
  resp = await WeatherClient.forecast(lat,long,exclude=blocks)
  print(resp)
  weather = resp['currently']
  xtemp,_,_ = await getTemp()
  embed=discord.Embed(title="Weather", description=f"**{weather['summary']}**  Requested by {message.author.mention}", color=discord.Color.green())
  embed.add_field(name='Temperature', value=f'**Probe**: {round(xtemp,2)}°C\n**Report**: {weather["temperature"]}°C\n', inline=True)
  embed.add_field(name='Time:', value=f"**Local**:{datetime.now().strftime('%H:%M:%S')}\n**Report**: {datetime.fromtimestamp(weather['time']).strftime('%H:%M:%S')}", inline=True)
...
commands = {
  'effects':[[],cEffects,False, False],
  'gif':[['gfy','gfycat'],cGif,True, False],
  'pic':[['picture'],cPic,True, False],
...
WeatherClient = aioDarkSkyAPI.Client(weather_secret,loop=client.loop, session=client.session)
...
async def on_ready():
  ...
  await client.change_presence(activity=discord.Activity(name='the dog',type=discord.ActivityType.watching))
